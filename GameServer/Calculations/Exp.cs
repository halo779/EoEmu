...
namespace GameServer.Calculations
{
    ...
    public partial class Calculation
    {
                    ...
                    if (CanSee(Member.Value.Client.X, Member.Value.Client.Y, CSocket.Client.X, CSocket.Client.Y) && Member.Value.Client.Level < 135 && Member.Value.Client.ID != CSocket.Client.ID)
                    {
                        ...
                        if (ExpToGive > 0)
                        {
                            ...
                            if (noobexp && ((int)Member.Value.Client.Class >= 133 && (int)Member.Value.Client.Class <= 135))
                                Member.Value.Send(ConquerPacket.Chat(0, "SYSTEM", Member.Value.Client.Name, "You gained " + ExpToGive + " exp from team killings, with additional exp for low level team-mates, and for being a water tao.", Struct.ChatType.Top));
                            else if (!noobexp && ((int)Member.Value.Client.Class >= 133 && (int)Member.Value.Client.Class <= 135))
                                Member.Value.Send(ConquerPacket.Chat(0, "SYSTEM", Member.Value.Client.Name, "You gained " + ExpToGive + " exp from team killings, with additional exp for low level team-mate.", Struct.ChatType.Top));
                            else
                                Member.Value.Send(ConquerPacket.Chat(0, "SYSTEM", Member.Value.Client.Name, "You gained " + ExpToGive + " exp from team killings.", Struct.ChatType.Top));
        ...
        public static void GiveLevel(ClientSocket CSocket)
        {
            ...
            CSocket.Send(ConquerPacket.Chat(0, "SYSTEM", CSocket.Client.Name, "Congratulations! You have attained level " + CSocket.Client.Level + ". Keep going!", Struct.ChatType.Top));
